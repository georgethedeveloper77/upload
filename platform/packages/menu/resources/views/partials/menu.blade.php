<ol class="dd-list">
    @foreach ($menu_nodes as $key => $row)
        <li class="dd-item dd3-item @if ($row->reference_id > 0) post-item @endif"
            data-reference-type="{{ $row->reference_type }}"
            ...
            <div class="dd3-content">
                ...
                <span
                    class="text float-right">{{ $row->reference_type ? $row->reference_type : trans('packages/menu::menu.custom_link') }}</span>
            ...
            <div class="item-details">
                <label class="pad-bot-5">
                    <span class="text pad-top-5 dis-inline-block"
                          data-update="title">{{ trans('packages/menu::menu.title') }}</span>
                ...
                @if (!$row->reference_id)
                    <label class="pad-bot-5 dis-inline-block">
                        <span class="text pad-top-5"
                              data-update="custom-url">{{ trans('packages/menu::menu.url') }}</span>
                    ...
                    <div style="width: 228px; display: inline-block">
                        <div class="ui-select-wrapper">
                            <select name="target" class="ui-select" id="target" data-old="{{ $row->target }}">
                                <option value="_self"
                                        @if ($row->target == '_self') selected="selected" @endif>{{ trans('packages/menu::menu.self_open_link') }}
                                </option>
                                <option value="_blank"
                                        @if ($row->target == '_blank') selected="selected" @endif>{{ trans('packages/menu::menu.blank_open_link') }}
                                </option>
                            </select>
