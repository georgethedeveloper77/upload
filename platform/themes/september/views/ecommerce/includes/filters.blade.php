@php
    use Botble\Base\Enums\BaseStatusEnum;use Botble\Ecommerce\Repositories\Interfaces\ProductTagInterface;$brands = get_all_brands(['status' => BaseStatusEnum::PUBLISHED], ['slugable'], ['products']);
    $categories = get_product_categories(['status' => BaseStatusEnum::PUBLISHED], ['slugable'], ['products'], true);
    $tags = app(ProductTagInterface::class)->allBy(['status' => BaseStatusEnum::PUBLISHED], ['slugable'])
@endphp
...
<aside class="widget widget--shop">
    ...
    <div class="widget__content">
        <ul class="widget__links list--plus">
            <li @if (URL::current() == route('public.products')) class="active" @endif><a
                    href="{{ route('public.products') }}">{{ __('All Products') }}</a></li>
            @foreach($categories as $category)
                <li @if (URL::current() == $category->url) class="active" @endif><a
                        href="{{ $category->url }}">{{ $category->name }} ({{ $category->products_count }})</a></li>
            @endforeach
        </ul>
    </div>
</aside>
        ...
        <div class="widget__content">
            <ul class="widget__links list--plus">
                @foreach($brands as $brand)
                    <li @if (URL::current() == $brand->url) class="active" @endif><a
                            href="{{ $brand->url }}">{{ $brand->name }} ({{ $brand->products_count }})</a></li>
                @endforeach
            </ul>
        </div>
    </aside>
...
<aside class="widget widget--shop widget--tags">
    ...
    <div class="widget__content">
        <ul class="list--tags">
            @foreach($tags as $tag)
                <li @if (URL::current() == $tag->url) class="active" @endif><a
                        href="{{ $tag->url }}">{{ $tag->name }}</a></li>
            @endforeach
        </ul>
    </div>
</aside>
