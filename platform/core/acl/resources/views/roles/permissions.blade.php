<ul id='auto-checkboxes' data-name='foo' class="list-unstyled list-feature">
    <li id="mainNode">
        ...
        <label for="expandCollapseAllTree"
               class="label label-default allTree">{{ trans('core/acl::permissions.all') }}</label>
        <ul>
            @foreach ($children['root'] as $elementKey => $element)
                <li class="collapsed" id="node{{ $elementKey }}">
                    <input type="checkbox" class="hrv-checkbox" id="checkSelect{{ $elementKey }}" name="flags[]"
                           value="{{ $flags[$element]['flag'] }}"
                           @if (in_array($flags[$element]['flag'], $active)) checked @endif>
                    <label for="checkSelect{{ $elementKey }}" class="label label-warning"
                            ...
                            @foreach($children[$element] as $subKey => $subElements)
                                <li class="collapsed" id="node_sub_{{ $elementKey  }}_{{ $subKey }}">
                                    <input type="checkbox" class="hrv-checkbox"
                                           id="checkSelect_sub_{{ $elementKey  }}_{{ $subKey }}" name="flags[]"
                                           value="{{ $flags[$subElements]['flag'] }}"
                                           @if (in_array($flags[$subElements]['flag'], $active)) checked @endif>
                                    <label for="checkSelect_sub_{{ $elementKey  }}_{{ $subKey }}"
