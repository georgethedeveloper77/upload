    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <a class="float-left btn btn-danger" href="#"
                   id="hide-widget-confirm-bttn">{{ trans('core/dashboard::dashboard.confirm_hide_btn') }}</a>
                <button class="float-right btn btn-primary"
                        data-dismiss="modal">{{ trans('core/dashboard::dashboard.cancel_hide_btn') }}</button>
            </div>
        </div>
    </div>
</div>
            ...
            <div class="modal-body with-padding">
                @foreach ($widgets as $widget)
                    ...
                    <section class="wrap_{{ $widget->name }}">
                        <i class="{{ $widget->icon ?? 'box_img_sale ' . $widget->name }} @if ($widget_setting && $widget_setting->status == 0) widget_none_color @endif"
                           style="background-color: {{ $widget->color ?? '#7c87b6' }}"></i>
