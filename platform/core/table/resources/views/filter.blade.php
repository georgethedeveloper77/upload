<div class="wrapper-filter">
                    ...
                    <select name="filter_columns[]" class="ui-select filter-column-key">
                        ...
                        @foreach($columns as $columnKey => $column)
                            <option value="{{ $columnKey }}"
                                    @if ($filterItem['column'] == $columnKey) selected @endif>{{ $column['title'] }}</option>
                        @endforeach
                    </select>
                    <svg class="svg-next-icon svg-next-icon-size-16">
                        <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#select-chevron"></use>
    ...
    <div style="margin-top: 10px;">
        <a href="javascript:"
           class="btn btn-secondary add-more-filter">{{ trans('core/table::table.add_additional_filter') }}</a>
        <a href="{{ URL::current() }}"
           class="btn btn-info @if (!request()->has('filter_table_id')) hidden @endif">{{ trans('core/table::table.reset') }}</a>
