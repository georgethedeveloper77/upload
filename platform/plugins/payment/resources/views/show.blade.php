        ...
        <div class="col-md-9">
            <div class="widget meta-boxes">
                ...
                <div class="widget-body">
                    <p>{{ trans('plugins/payment::payment.created_at') }}: <strong>{{ $payment->created_at }}</strong>
                    </p>
                    <p>{{ trans('plugins/payment::payment.payment_channel') }}:
                        <strong>{{ $payment->payment_channel->label() }}</strong></p>
                    <p>{{ trans('plugins/payment::payment.total') }}:
                        <strong>{{ $payment->amount }} {{ $payment->currency }}</strong></p>
                    <p>{{ trans('plugins/payment::payment.status') }}:
                        <strong>{!! $payment->status->label() !!}</strong></p>
                    {!! $detail !!}
                </div>
            </div>
            ...
            <div class="widget meta-boxes">
                <div class="widget-title">
                    <h4><label for="status" class="control-label required"
                               aria-required="true">{{ trans('core/base::tables.status') }}</label></h4>
                </div>
                <div class="widget-body">
