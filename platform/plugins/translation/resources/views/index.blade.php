    ...
    <div class="widget meta-boxes">
        ...
        <div class="widget-body box-translation">
            @if (empty($group))
                ...
                <button type="submit"
                        class="btn btn-primary button-import-groups">{{ trans('plugins/translation::translation.import_group') }}</button>
                {!! Form::close() !!}
                <br>
            @endif
            @if (!empty($group))
                <form method="POST" action="{{ route('translations.group.publish', compact('group')) }}"
                    ...
                    <a href="{{ route('translations.index') }}"
                       class="btn btn-secondary translation-back">{{ trans('plugins/translation::translation.back') }}</a>
                </form>
                                ...
                                @foreach($locales as $locale)
                                    ...
                                    <td class="text-left">
                                        <a href="#edit"
                                           ...
                                           data-type="textarea" data-pk="{{ $item ? $item->id : 0 }}"
                                           data-url="{{ $editUrl }}"
