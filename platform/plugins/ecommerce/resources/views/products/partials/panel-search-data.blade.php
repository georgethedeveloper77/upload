        ...
        <ul class="clearfix">
            @if (!$availableProducts->isEmpty())
                @foreach($availableProducts as $availableProduct)
                    <li class="@if (!$includeVariation) selectable-item @endif"
                        @if (!$includeVariation) data-name="{{ $availableProduct->name }}"
                        data-image="{{ RvMedia::getImageUrl($availableProduct->image, 'thumb', false, RvMedia::getDefaultImage()) }}"
                        data-id="{{ $availableProduct->id }}"
                        data-url="{{ route('products.edit', $availableProduct->id) }}" @endif>
                        <div class="wrap-img inline_block vertical-align-t float-left"><img class="thumb-image"
                        ...
                        <label class="inline_block ml10 mt10 ws-nm"
                               style="width:calc(100% - 50px);">{{ $availableProduct->name }}</label>
                        @if ($includeVariation)
                            <div class="clear"></div>
                            <ul>
                                @foreach($availableProduct->variations as $variation)
                                    <li class="clearfix product-variant selectable-item"
