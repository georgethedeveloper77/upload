<li class="dropdown dropdown-extended dropdown-inbox" id="header_inbox_bar">
    ...
    <ul class="dropdown-menu dropdown-menu-right">
        ...
        <li>
            <ul class="dropdown-menu-list scroller" style="height: {{ count($orders) * 70 }}px;"
                data-handle-color="#637283">
                @foreach($orders as $order)
                    <li>
                        <a href="{{ route('orders.edit', $order->id) }}">
                            <span class="photo">
                                <img src="{{ \Botble\Base\Supports\Gravatar::image($order->address->email) }}"
                                     class="rounded-circle" alt="{{ $order->address->name }}">
                            </span>
                            <span class="subject"><span class="from"> {{ $order->address->name }} </span><span
