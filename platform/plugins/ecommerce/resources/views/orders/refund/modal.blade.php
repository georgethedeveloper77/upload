        ...
        <div class="next-form-grid-cell">
                            ...
                            <tr>
                                ...
                                <td class="text-right">
                                    @if ($orderProduct->qty - $orderProduct->restock_quantity > 0)
                                        <input class="j-refund-quantity width-50-px pl5 m-auto next-input p-none-r"
                                               name="products[{{ $orderProduct->product_id }}]" type="number"
                                               min="0"
            ...
            <div class="table-wrap p-none">
                <table class="refund-totals">
                                ...
                                <label class="next-label inline mt10">
                                    ...
                                    <span>
                                        {{ trans('plugins/ecommerce::order.return') }} <span
                                            class="total-restock-items">{{ $order->products->sum('qty') - $order->products->sum('restock_quantity') }}</span> {{ trans('plugins/ecommerce::order.products') }}
                                    </span>
                                </label>
                            </td>
                        </tr>
                    ...
                    @endif
                    <tr>
                        <td class="text-right">{{ trans('plugins/ecommerce::order.total_amount_can_be_refunded') }}:
                        </td>
                        <td class="text-right quantity text-no-bold">
                            ...
                            <td class="p-xs width-150-px">
                                <div class="next-input--stylized">
                                    <input type="text"
                                           ...
                                           data-target=".refund-amount-text" name="refund_amount"
    ...
    <div class="next-form-grid">
        <div class="next-form-grid-cell">
            <label class="text-title-field"
                   for="refund-note">{{ trans('plugins/ecommerce::order.refund_reason') }}</label>
            <div>
                <input type="text" class="next-input" name="refund_note" id="refund-note"
