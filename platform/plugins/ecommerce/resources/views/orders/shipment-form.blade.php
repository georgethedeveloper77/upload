<form class="shipment-create-panel" action="{{ route('orders.create-shipment', $order->id) }}">
    <div class="pd-all-20 pt10">
        <div class="flexbox-grid-form flexbox-grid-form-no-outside-padding">
            ...
            <div class="flexbox-grid-form-item max-width-220-px">
                <div class="form-group">
                    <div class="form-group">
                        <label
                            class="text-title-field">{{ trans('plugins/ecommerce::shipping.weight_unit', ['unit' => ecommerce_weight_unit()]) }}
                            :</label>
        ...
        <div class="flexbox-grid-form flexbox-grid-form-no-outside-padding rps-form-767 mb10">
            <div
                class="shipment-address-box-1 wrapword">@include('plugins/ecommerce::orders.shipping-address.line', ['address' => $order->address])</div>
        </div>
                    ...
                    <div class="user-control-combox-v3" id="select-shipping-provider">
                        <div class="flexbox-grid-default" data-toggle="dropdown" aria-expanded="false">
                            <div class="flexbox-content p-none-important">
                                <input type="hidden" name="method" value="{{ $order->shipping_method }}"
                                       class="input-hidden-shipping-method">
                                <input type="hidden" name="option" value="{{ $order->shipping_option }}"
                                    ...
                                    <tr>
                                        <th><b>{{ trans('plugins/ecommerce::shipping.packages') }}</b><i
                                                style="display:block;font-size: xx-small; color: darkgoldenrod"><span
                                                    class="ws-nm">{{ trans('plugins/ecommerce::shipping.warehouse') }} {{ get_ecommerce_setting('store_city') }}, {{ get_ecommerce_setting('store_state') }}</span></i>
                                        </th>
                                        <th class="text-right">
            ...
            <div
                ...
                <div class="flexbox-grid-form-item">
                    @if ($order->payment->payment_channel == \Botble\Payment\Enums\PaymentMethodEnum::COD && $order->payment->status !== \Botble\Payment\Enums\PaymentStatusEnum::COMPLETED)
                        <div class="form-group">
                            <label class="text-title-field">{{ trans('plugins/ecommerce::shipping.cod_amount') }}
                                :</label>
                            <div class="next-input--stylized">
                                <span
                                    class="next-input-add-on next-input__add-on--before">{{ get_application_currency()->symbol }}</span>
                                <input class="next-input next-input--invisible input-mask-number" name="cod_amount"
                                       type="text" value="{{ format_price($order->amount, true) }}">
                            </div>
                        </div>
                    @endif
                </div>
