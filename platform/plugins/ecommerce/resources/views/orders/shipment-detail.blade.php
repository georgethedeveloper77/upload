<div class="shipment-info-panel hide-print">
    <div class="shipment-info-header">
        ...
        <span
            class="label carrier-status carrier-status-{{ $shipment->status }}">{{ $shipment->status->label() }}</span>
    </div>
    ...
    <div class="pd-all-20 pt10">
        <div class="flexbox-grid-form flexbox-grid-form-no-outside-padding rps-form-767 pt10">
            ...
            <div class="flexbox-grid-form-item rps-no-pd-none-r ws-nm">
                ...
                <span><i>{{ $shipment->weight }} kg</i></span>
            </div>
        </div>
        <div class="flexbox-grid-form flexbox-grid-form-no-outside-padding rps-form-767 pt10">
            <div class="flexbox-grid-form-item ws-nm">
                <span>{{ trans('plugins/ecommerce::shipping.warehouse') }}:</span>
                <span><i>{{ $shipment->store->name }}</i></span>
            </div>
            ...
            <div class="flexbox-grid-default">
                @if (in_array($shipment->status, [\Botble\Ecommerce\Enums\ShippingStatusEnum::NOT_APPROVED, \Botble\Ecommerce\Enums\ShippingStatusEnum::APPROVED]))
                    <div class="flexbox-content">
                        <button type="button" class="btn btn-secondary btn-destroy btn-cancel-shipment"
                                data-action="{{ route('orders.cancel-shipment', $shipment->id) }}">{{ trans('plugins/ecommerce::shipping.cancel_shipping') }}</button>
                    </div>
                @endif
            </div>
        </div>
