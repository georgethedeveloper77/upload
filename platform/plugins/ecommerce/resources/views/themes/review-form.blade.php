    ...
    <div class="col-md-6 col-12">
        ...
        <div class="form-group {{ $errors->has('star') ? ' has-error' : '' }}">
            ...
            <input type="hidden" name="star" id="rating"
                   @if (!auth('customer')->check() || ($is_reviewed = check_if_reviewed_product($productId))) disabled
                   value="{{ get_customer_reviewed_value($productId) }}" @endif/>
        </div>
        ...
        @if (!isset($is_reviewed) || !$is_reviewed)
            ...
            @else
                <p class="text-danger">{{ __('Please') }} <a
                        href="{{ route('customer.login') }}">{{ __('login') }}</a> {{ __('to write review!') }}</p>
            @endif
        @endif
        {!! Form::close() !!}
    </div>
                    ...
                    @if (!empty($review->user) && !empty($review->product))
                        <li>
                            <p>
                                ...
                                -
                                <span
                                    class="review-time">{{ BaseHelper::formatDate($review->created_at, 'Y-m-d H:i:s') }}</span>
                            ...
                            @if (auth('customer')->check() && auth('customer')->user()->getAuthIdentifier() == $review->customer_id)
                                <span class="delete-review-btn"><a
                                        href="{{ route('public.reviews.destroy', $review->id) }}"
                                        title="{{ trans('plugins/ecommerce::review.delete_review') }}"><i
