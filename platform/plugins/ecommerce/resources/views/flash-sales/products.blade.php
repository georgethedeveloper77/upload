<div class="form-group">
    <input type="hidden" name="products"
           value="@if ($flashSale->id) {{ implode(',', $flashSale->products()->allRelatedIds()->toArray()) }} @endif"/>
    <div class="box-search-advance product">
        <div>
            <input type="text" class="next-input textbox-advancesearch"
                   ...
                   data-target="{{ route('products.get-list-product-for-search', 0) }}">
        </div>
    ...
    <div class="list-selected-products @if (!$products->count()) hidden @endif">
        <div class="mt20"><label class="text-title-field">{{ trans('plugins/ecommerce::products.selected_products') }}
                :</label></div>
                ...
                @foreach($products as $index => $product)
                    <tr data-product-id="{{ $product->id }}">
                        ...
                        <td class="pl5 p-r5 min-width-200-px" style="padding-top: 15px;">
                            <a class="hover-underline pre-line" href="{{ route('products.edit', $product->id) }}"
                        ...
                        <td class="pl5 p-r5 text-right width-20-px min-width-20-px" style="padding-top: 15px;">
                            <a href="#" class="btn-trigger-remove-selected-product"
                               title="{{ trans('plugins/ecommerce::products.delete') }}" data-id="{{ $product->id }}">
                    ...
                    <tr data-product-id="{{ $product->id }}">
                        <td colspan="3"
                            style="border-top: none; @if (!$loop->last) border-bottom: 1px solid #ececec; @endif">
                            <div class="row">
                                <div class="col-6">
                                    <div class="form-group">
                                        <label
                                            class="control-label required">{{ trans('plugins/ecommerce::products.price') }}</label>
                                        <input type="text" class="form-control input-mask-number"
