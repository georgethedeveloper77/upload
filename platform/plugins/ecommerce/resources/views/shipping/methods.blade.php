        ...
        <div class="group">
            <div class="row">
                <div class="@if (count($shipping) > 0) col-md-3 col-sm-12 @else col-sm-12 @endif">
                    ...
                    <p><a href="#" class="btn btn-secondary" data-fancybox data-type="ajax"
                          data-src="{{ route('shipping_methods.region.create') }}">{{ trans('plugins/ecommerce::shipping.select_country') }}</a>
                    </p>
                </div>
                                ...
                                <div class="wrap-table-shipping-{{ $shippingItem->id }}">
                                    <div class="pd-all-20 p-none-b">
                                        <label class="p-none-r">{{ trans('plugins/ecommerce::shipping.country') }}:
                                            <strong>{{ Arr::get(\Botble\Base\Supports\Helper::countries(), $shippingItem->title, $shippingItem->title) }}</strong></label>
                                        <a href="#"
                                           class="btn-change-link float-right pl20 btn-add-shipping-rule-trigger"
                                           data-shipping-id="{{ $shippingItem->id }}">{{ trans('plugins/ecommerce::shipping.add_shipping_rule') }}</a>
                                        &nbsp;&nbsp;&nbsp;
                                        <a href="#"
                                           class="btn-change-link float-right excerpt btn-confirm-delete-region-item-modal-trigger text-danger"
                                           data-id="{{ $shippingItem->id }}"
                                           data-name="{{ $shippingItem->title }}">{{ trans('plugins/ecommerce::shipping.delete') }}</a>
                                    </div>
